<x-layouts.admin :title="'Modifier ' . $category->name . ' — Admin'">
    ...
    <div class="fixed inset-0 z-40 overflow-y-auto bg-ink/50 px-4 py-8 sm:py-12">
        <div class="mx-auto max-w-md rounded-xl border border-line bg-white p-5 shadow-2xl sm:p-6">
            <div class="mb-6 flex items-start justify-between gap-4">
                <h1 class="font-display font-bold text-2xl text-navy">Modifier « {{ $category->name }} »</h1>
        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}">
            ...
            <input type="text" name="name" value="{{ old('name', $category->name) }}" class="w-full rounded-lg border-line text-sm" required>
